...
class basePage {
  ...
  async crop(fileName, left, top, width, height) {
    sharp('Reports/screenshots/' + fileName +'.jpg')
    .extract({ left: left, top: top, width: width, height: height })
    .toFile('Reports/screenshots/' + fileName +'_New.jpg', function (err) {
        if (err) console.log(err);
    })
  }

  async valueQR(fileName) {
    const buffer = fs.readFileSync('Reports/screenshots/' + fileName +'.jpg');
    ...
    return new Promise((resolve, reject) => { 
      ...
      qrCodeInstance.decode(img.bitmap);
    });
  }

  async downloadImage(imageUrl, fileName) {
    const file = fs.createWriteStream('Reports/screenshots/' + fileName +'.jpg');
